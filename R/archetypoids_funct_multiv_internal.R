#' archetypoids_funct_multiv_internal
#' 
#' @rdname archetypoids_funct_multiv_internal
#'
#' @description 
#' These three functions are called by the archetypoid algorithm 
#' (\code{\link{archetypoids_funct_multiv}}). All these functions are therefore not 
#' solely used.
#' 
#' @author 
#' Guillermo Vinue
...
#' @noRd
swap_funct_multiv <- function(vect_arch_ini, rss_arch_ini, huge = 200, numArchoid, x_gvv, n, PM){
  ...
  for (l in 1:numArchoid) {
    ...
    for (i in setpossibles) {
      ...
      rss1[i] <- frobenius_norm_funct_multiv(resid, PM) / n
  ...
  if (numArchoid == 1) {
    result <- swap2_k1_funct_multiv(vect_arch_end, vect_arch_ini, rss, huge, numArchoid, x_gvv, n, PM)
  }else{
    result <- swap2_funct_multiv(vect_arch_end, vect_arch_ini, rss, huge, numArchoid, x_gvv, n, PM)
  } 
  
  return(result)
}

#' Further swap function of the functional multivariate archetypoid analysis 
...
#' @return Optimal vector of archetypoids.
#' @noRd
swap2_funct_multiv <- function(vect_arch_end, vect_arch_ini, rss, huge = 200, numArchoid, x_gvv, n, PM){
...
#' @return Optimal vector of archetypoids.
#' @noRd
swap2_k1_funct_multiv <- function(vect_arch_end, vect_arch_ini, rss, huge=200, numArchoid, x_gvv, n, PM){
