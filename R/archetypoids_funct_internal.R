#' archetypoids_funct_internal
#' 
#' @rdname archetypoids_funct_internal
#'
#' @description 
#' These three functions are called by the archetypoid algorithm 
#' (\code{\link{archetypoids_funct}}). All these functions are therefore not 
#' solely used.
#' 
#' @author 
#' Guillermo Vinue
#' 
#' @seealso 
#' \code{\link{archetypoids_funct}}
#'                  
#' General swap function of the functional archetypoid analysis 
...
#' @noRd
swap_funct <- function(vect_arch_ini, rss_arch_ini, huge = 200, numArchoid, x_gvv, n, PM){
  ...
  for (l in 1:numArchoid) {
    ...
    for (i in setpossibles) {
      ...
      rss1[i] <- frobenius_norm_funct(resid, PM) / n
  ...
  if (numArchoid == 1) {
    result <- swap2_k1_funct(vect_arch_end, vect_arch_ini, rss, huge, numArchoid, x_gvv, n, PM)
  }else{
    result <- swap2_funct(vect_arch_end, vect_arch_ini, rss, huge, numArchoid, x_gvv, n, PM)
  } 
  
  return(result)
}

#' Further swap function of the functional archetypoid analysis 
...
#' @return Optimal vector of archetypoids.
#' @noRd
swap2_funct <- function(vect_arch_end, vect_arch_ini, rss, huge = 200, numArchoid, x_gvv, n, PM){
...
#' @return Optimal vector of archetypoids.
#' @noRd
swap2_k1_funct <- function(vect_arch_end, vect_arch_ini, rss, huge=200, numArchoid, x_gvv, n, PM){
