...
stepArchetypesRawData_funct <- function(data, numArch, numRep = 3, verbose = TRUE, 
                                        saveHistory = FALSE, PM){
  ...
  for (i in 1:length(numArch)) {
    ...
    for (j in seq_len(numRep)) {
      if (verbose) 
        ...
        as[[i]][[j]] <- archetypes_funct(data, k = numArch[i], saveHistory = FALSE, 
                                         family = archetypesFamily("original",
                                                          scalefn = no.scalefn, 
                                                          rescalefn = no.rescalefn,
                                                          normfn = frobenius_norm_funct), 
                                         PM = PM)
    }
  }
