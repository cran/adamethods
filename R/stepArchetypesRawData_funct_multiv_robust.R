...
stepArchetypesRawData_funct_multiv_robust <- function(data, numArch, numRep = 3, verbose = TRUE, 
  ...
  for (i in 1:length(numArch)) {
    ...
    for (j in seq_len(numRep)) {
      if (verbose) 
        ...
        as[[i]][[j]] <- archetypes_funct_multiv_robust(data, k = numArch[i], saveHistory = FALSE, 
                                                    family = archetypesFamily("original",
                                                          scalefn = no.scalefn, 
                                                          rescalefn = no.rescalefn,
                                                          normfn = frobenius_norm_funct_multiv_robust,
