#' archetypoids_funct_robust_internal
#' 
#' @rdname archetypoids_funct_robust_internal
#'
#' @description 
#' These three functions are called by the archetypoid algorithm 
#' (\code{\link{archetypoids_funct_robust}}). All these functions are therefore not 
#' solely used.
#' 
#' @author 
#' Irene Epifanio
...
swap_funct_robust <- function(vect_arch_ini, rss_arch_ini, huge = 200, numArchoid, x_gvv, n, PM, prob){
  ...
  for (l in 1:numArchoid) {
    ...
    for (i in setpossibles) {
      #if (length(c(i,vect_arch_ini[-l])) < numArchoid) {
      #  next
      #}
      ...
      for (j in 1:n) {
        #if(length(coef(nnls(zs, x_gvv[,j]))) != numArchoid){
        # print(c(i,vect_arch_ini[-l])) 
        # # print(j) ; print(zs) ; print(x_gvv[,j])
        #}
        alphas[, j] = coef(nnls(zs, x_gvv[,j]))
      }
      
      resid <- zs[1:(nrow(zs) - 1),] %*% alphas - x_gvv[1:(nrow(x_gvv) - 1),]
      rss1[i] <- frobenius_norm_funct_robust(resid, PM, prob) / n
  ...
  if (numArchoid == 1) {
    result <- swap2_k1_funct_robust(vect_arch_end, vect_arch_ini, rss, huge, numArchoid, x_gvv, n, PM, prob)
  }else{
    result <- swap2_funct_robust(vect_arch_end, vect_arch_ini, rss, huge, numArchoid, x_gvv, n, PM, prob)
  } 
  
  return(result)
}

#' Further swap function of the functional robust archetypoid analysis 
...
#' @return Optimal vector of archetypoids.
#' @noRd
swap2_funct_robust <- function(vect_arch_end, vect_arch_ini, rss, huge = 200, numArchoid, x_gvv, n, PM, prob){
...
#' @return Optimal vector of archetypoids.
#' @noRd
swap2_k1_funct_robust <- function(vect_arch_end, vect_arch_ini, rss, huge=200, numArchoid, x_gvv, n, PM, prob){
