#' Perform multivariate robust functional archetypal analysis on a data matrix.
#'
...
#'
#' @examples
#' # Similarly with archetypes_funct_multiv_robust.
#' library(archetypes)
#' data(toy)
#' a <- archetypes(toy, 3)
#' str(a)
...
archetypes_funct_multiv_robust <- function(data, k, weights = NULL, maxIterations = 100, 
                                        minImprovement = sqrt(.Machine$double.eps), 
                                        maxKappa = 1000, verbose = FALSE, saveHistory = FALSE, 
                                        family = archetypesFamily("original"), 
                                        PM = PM, prob = prob, nbasis = nbasis, nvars = nvars,...) 
{
  mycall <- match.call()
  famargs <- list(...)
  memento <- NULL
  ...
  rss <- family$normfn(resid, PM, prob, nbasis, nvars, ...)/n
  reweights <- rep(1, n)
  ...
  tryCatch(while ((i <= maxIterations) & (imp >= minImprovement)) {
    ...
    rss1 <- family$normfn(resid1n, PM, prob, nbasis, nvars, ...)/n
    ...
    rss2 <- family$normfn(resid, PM, prob, nbasis, nvars, ...)/n
    imp <- rss - rss2
    rss <- rss2
    kappas <- c(alphas = kappa(alphas), betas = kappa(betas), 
